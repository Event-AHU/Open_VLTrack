...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Score TNL2K Qwen refine candidates with tracker gain and export SFT dataset."
    )
    ...
    parser.add_argument("--tnl2k-root", default="", help="Optional TNL2K root override")
...
def build_seq_map(split, tnl2k_root=""):
    env = EnvironmentSettings()
    root = tnl2k_root or env.tnl2k_dir
    dataset = TNL2k_Lang(root=root, split=split)
    seq_map = {}
    for seq_id, seq_name in enumerate(dataset.sequence_list):
        ...
        frames = [dataset._get_frame_path(seq_path, seq_name, i) for i in range(int(info["bbox"].shape[0]))]
        seq_map[seq_name] = {
            "name": seq_name,
            "frames": frames,
            "ground_truth_rect": info["bbox"].cpu().numpy(),
            "object_class": seq_name.split("/")[-1],
        }
    return seq_map
...
def main():
    ...
    seq_map = build_seq_map(split=args.split, tnl2k_root=args.tnl2k_root)
