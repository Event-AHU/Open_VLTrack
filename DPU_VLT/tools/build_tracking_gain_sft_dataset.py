...
def parse_args():
    parser = argparse.ArgumentParser(description="Score Qwen refine candidates with tracker gain and export SFT dataset.")
    ...
    parser.add_argument("--split", default="", choices=["", "train", "val", "test"], help="TNLLT split for evaluation. Empty means infer from candidates meta.")
    ...
    parser.add_argument("--checkpoint", default="", help="Optional checkpoint override. If empty, use default from parameter builder.")
    ...
    parser.add_argument("--gain-th", type=float, default=0.02)
...
def build_seq_map(split, tnllt_root=""):
    env = EnvironmentSettings()
    root = tnllt_root or env.tnllt_dir
    dataset = TNLLT(root=root, split=split)
    seq_map = {}
    for seq_id, seq_name in enumerate(dataset.sequence_list):
        ...
        seq_map[seq_name] = SimpleNamespace(
            name=seq_name,
            frames=frames,
...
def build_sft_record(entry, selected_text, gain, args):
    return {
        ...
        "messages": [
            {
                "role": "system",
                "content": SYSTEM_PROMPTS[args.system_prompt_version],
            },
            {
                "role": "user",
...
def run_tracking_window(tracker, seq, start_frame, text_struct, window_size, args):
    tracker.sequence_text_description = text_struct

    image = tracker._read_image(seq.frames[start_frame]) if hasattr(tracker, "_read_image") else None
    if image is None:
        import cv2
...
def score_anchor(entry, seq, tracker, args):
    ...
    baseline_score, baseline_ious = run_tracking_window(tracker, seq, frame_id, baseline_text, args.window_size, args)
    ...
    for idx, candidate in enumerate(candidates):
        ...
        cand_score, cand_ious = run_tracking_window(tracker, seq, frame_id, cand_text, args.window_size, args)
    ...
    if scored_candidates:
        best = max(scored_candidates, key=lambda x: x["gain"])
...
def main():
    ...
    seq_map = build_seq_map(split=split, tnllt_root=args.tnllt_root)
    ...
    print('[INFO] Building tracker once per process...')
