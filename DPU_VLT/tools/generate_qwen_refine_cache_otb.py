...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate offline Qwen refine cache for OTB99-Lang.")
...
def validate_output(raw_output, parsed, max_field_words, max_growth):
    def _check(original, updated, name):
        ...
        return updated
    return {
        "concept": _check(str(parsed.get("concepts", "")), str(raw_output.get("concept", raw_output.get("concepts", ""))).strip(), "concept"),
        "background": _check(str(parsed.get("background", "")), str(raw_output.get("background", "")).strip(), "background"),
    }
...
def run_qwen(processor, model, image_rgb, system_prompt, user_prompt, device, max_new_tokens):
    ...
    return processor.batch_decode(generated[:, prompt_len:], skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]


def get_anchor_ids(num_frames, interval, offset):
    return list(range(max(0, offset), num_frames, interval))
...
def main():
    ...
    if data is None:
        data = {
            "meta": {
                ...
                "dataset": "OTB99Lang", "split": args.split,
                "parsed_text_path": args.parsed_text_path,
                "cache_interval": args.cache_interval, "anchor_offset": args.anchor_offset,
            },
            "entries": {},
        }
    ...
    for seq_id in seq_ids:
        ...
        # OTB: same parsed text for all frames in a sequence
        parsed = dataset.get_language_description(seq_id)
        ...
        for frame_id in anchor_ids:
            ...
            try:
                ...
                raw_text = run_qwen(processor, model, image_rgb, system_prompt, build_user_prompt(parsed), args.device, args.max_new_tokens)
        ...
        print(f"[INFO] seq={seq_name} written={written} total={len(data['entries'])}")
        if args.limit_total > 0 and written >= args.limit_total:
            break

    print(f"[DONE] written={written}, skipped={skipped_existing}, total={len(data['entries'])}, output={output_path}")
