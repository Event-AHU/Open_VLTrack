...
class Fast_iTPN(BaseBackbone):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=512, depth_stage1=3, depth_stage2=3, depth=24,
        ...
        # 新增：语义特征融合层（依存 parser 输出 + 原文本特征）
        # 假设语义编码器输出维度为 (D+F) = 512 + 256 = 768（根据实际调整）
        self.semantic_fusion = nn.Sequential(
            nn.Linear(embed_dim + 768, embed_dim),  # 融合原文本特征(embed_dim)和语义特征(768)
            nn.GELU(),
    ...
    def _l_feat(self,l,semantic_feat=None):
        """
        生成文本特征并融合依存语义特征
        Args:
            l: 文本描述列表
            semantic_feat: 依存语义编码器输出 [B, 768]（新增参数）
        """
        ...
        l_emb = self.descript_embedding(descript_id_tensor.to('cuda'))
        l_emb += self.description_patch_pos_embed(l_emb)
        l_feat = l_emb
        # 2. 融合依存语义特征（如果提供）
        if semantic_feat is not None:
            # 确保语义特征与文本特征维度匹配
            # 语义特征投影：[B, 512] → [B, 1, D]（D为文本嵌入维度）
            semantic_proj = nn.Linear(512, l_feat.shape[-1], device=l_feat.device)
            semantic_emb = semantic_proj(semantic_feat).unsqueeze(1)  # [B, 1, D]
            
            # 扩展到文本序列长度：[B, 1, D] → [B, T, D]（与文本特征长度一致）
            semantic_emb = semantic_emb.repeat(1, l_feat.shape[1], 1)  # [B, T, D]
            
            # 融合策略：文本特征与语义特征逐元素相加（或拼接后投影，根据效果选择）
            # 方案1：相加融合（轻量，保留原始维度）
            l_feat = l_feat + semantic_emb  # [B, T, D]
            
            # 方案2：拼接后投影（更灵活，需额外参数）
            # concat_feat = torch.cat([l_feat, semantic_emb], dim=-1)  # [B, T, 2D]
            # l_feat = self.semantic_fusion(concat_feat)  # [B, T, D]，需在__init__中定义self.semantic_fusion
        return l_feat
    ...
    def forward_features(self, z, x, l, temporal_query=None, top_K=None,semantic_feat=None):
        B = x.shape[0]
        z_feat = self._z_feat(z,B)
        x_feat = self._x_feat(x)
        l_feat = self._l_feat(l,semantic_feat)
    ...
    def forward(self, z, x, l, temporal_query, top_K, semantic_feat=None,**kwargs):
        """
        ...
        Returns:
            ...
            attn : None
        """
        x, aux_dict = self.forward_features(z, x, l, temporal_query, top_K,semantic_feat=semantic_feat)
